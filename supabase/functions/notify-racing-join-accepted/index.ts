/**
 * Instant Web Push to the player when an admin accepts their Racing join request.
 *
 * Preferred: client invokes after successful racing_admin_approve_join.
 *
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isServiceRole && !isCron) {
      ...
      const { data: canHandle, error: permErr } = await userClient.rpc("racing_can_handle_joins", {
    ...
    const payload = JSON.stringify({
      ...
      url: `/(app)/competition/${jr.competition_id}`,
    });
