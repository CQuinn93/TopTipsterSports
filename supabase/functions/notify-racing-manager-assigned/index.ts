/**
 * Instant Web Push when a player is assigned as a Racing competition manager.
 *
 * Preferred: client invokes after successful racing_set_competition_manager(..., true).
 *
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: assignment, error: asErr } = await admin
      .from("racing_competition_managers")
    ...
    if (!isServiceRole && !isCron) {
      ...
      const { data: perm, error: permErr } = await userClient.rpc("racing_can_manage_competition", {
