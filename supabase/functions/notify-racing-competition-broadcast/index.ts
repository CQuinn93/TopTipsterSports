/**
 * Custom Web Push from Racing competition creator/Owner to all participants.
 *
 * Preferred: client invokes after composing title + body in Admin → Notify.
 *
 * Auth: signed-in creator/Owner via racing_admin_authorize_broadcast.
 *
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: prepared, error: prepErr } = await userClient.rpc(
      "racing_admin_authorize_broadcast",
      {
